...
export default function VerificationSentPageCard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (!user) {
        // If there is no user session, don't attempt to poll
        return;
      }
  ...
  const handleResend = async () => {
    ...
    setIsResending(true);
    try {
      // Directs Firebase to redirect Tab 2 to our custom closer route upon clicking "Continue"
